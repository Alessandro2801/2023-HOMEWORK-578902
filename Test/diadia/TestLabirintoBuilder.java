...
public class TestLabirintoBuilder {
	...
	public void testMonolocaleConAttrezzo() {
		Labirinto monolocale = labirintoBuilder
				.addStanzaIniziale(nomeStanzaIniziale).addAttrezzo("spada",1, nomeStanzaIniziale)
				.addStanzaVincente(nomeStanzaIniziale).addAttrezzo("spadina", 3, nomeStanzaIniziale)
				.getLabirinto();
		assertEquals(nomeStanzaIniziale,monolocale.getStanzaCorrente().getNome());
		assertEquals(nomeStanzaIniziale,monolocale.getStanzaVincente().getNome());
	...
	public void testMonolocaleConAttrezzoSingoloDuplicato() {
		Labirinto monolocale = labirintoBuilder
				.addStanzaIniziale(nomeStanzaIniziale)
				.addAttrezzo("spada",1, nomeStanzaIniziale)
				.addAttrezzo("spada",1, nomeStanzaIniziale)
				.getLabirinto();
	...
	public void testBilocale() {
		Labirinto bilocale = labirintoBuilder
				...
				.getLabirinto();
		assertEquals(bilocale.getStanzaVincente().getNome(),bilocale.getStanzaCorrente().getStanzaAdiacente("nord").getNome());
		assertTrue(bilocale.getStanzaCorrente().getDirezioni().contains("nord"));
	...
	public void testPiuDiQuattroAdiacenti() {
		...
		assertTrue(maze.getStanzaCorrente().getMappaStanzeAdiacenti().size()== 4);
		assertTrue(!maze.getStanzaCorrente().getMappaStanzeAdiacenti().containsValue(test));
	}
	...
	public void testAggiuntaAttrezzoAStanze_AppenaAggiunteMultiple() {
		...
		assertEquals(attrezzo,attrezzi.get(attrezzi.indexOf(attrezzo)));  // DA GUARDARE PER CAMBIARE
	}
	...
	public void testLabirintoConStanzaBloccata_ConPassepartout() {
		...
		assertEquals(nomeStanzaVincente,labirintoBuilder.getMappaStanze().get("stanza bloccata").getStanzaAdiacente("nord").getNome());	
	}
	...
	public void testLabirintoConStanzaBloccata_SenzaPassepartout() {
		...
		assertEquals("stanza bloccata",labirintoBuilder.getMappaStanze().get("stanza bloccata").getStanzaAdiacente("nord").getNome());
	}
	...
	public void testLabirintoCompletoConTutteLeStanze() {
		...
		assertEquals(mapAdiacenti.get("nord").getNome(),corridoio.getMappaStanzeAdiacenti().get("nord").getNome());  // PROBLEMA
